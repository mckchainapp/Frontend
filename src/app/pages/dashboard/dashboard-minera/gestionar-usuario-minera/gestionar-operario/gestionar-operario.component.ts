...
@Component({
  selector: 'app-gestionar-operario',
  templateUrl: './gestionar-operario.component.html',
  styleUrls: []
})
export class GestionarOperarioComponent implements OnInit {
  ...
  public operarioRequestForm = this.fb.group({
  ...
  operarioRequest(): void {
    ...
    var email: any = {
      emailUsuario: this.operarioRequestForm.controls['emailUsuario'].value,
    }
    ...
    this.gestionarusuario.postOperarioSolicitud(this.id, email).subscribe(
      data=>{
